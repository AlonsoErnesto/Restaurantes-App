...
export default function InfoUser(props){
    ...
    const changeAvatar = async() =>{
        ...
        const resultPermision = await ImagePicker.requestMediaLibraryPermissionsAsync()
        //await Permissions.askAsync(Permissions.CAMERA_ROLL);
        const resultPermisionCamera = await ImagePicker.launchImageLibraryAsync()
        //resultPermision.permissions.cameraRoll.status;

        if(resultPermisionCamera === "denied"){
            toastRef.current.show("Es necesario aceptar los permisos de la geleria.")
        }else{
